...
| `GqrxStatus`, `GqrxBookmarkSummary` | Whether Gqrx is running on the Mac and the UDP port AntennaHead receives its audio on (mirrors `gqrxFormHTML()`), and Gqrx's own bookmarks. The rest of the remote-control panel stays web-only. |
...
| `StartGqrxRequest`, `PlayGqrxBookmarkRequest`, `StartAudioFilesRequest`, `StartTextToSpeechRequest`, `StartRSSHeadlinesRequest` | Request bodies for the Listen buttons on the Devices sub-pages. |
